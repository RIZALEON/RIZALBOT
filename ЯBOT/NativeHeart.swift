...
enum NativeHeart {
    ...
    /// `maxNewTokens` (0.3.0, gamewrite): macOS llama-completion `-n`; default 96 keeps every old call identical.
    /// iOS dial-in keeps its own 32-token cap (n_ctx 512) — gamewrite falls back to starters there.
    static func generate(prompt: String, maxNewTokens: Int = 96) -> String {
    ...
    // Dial-in parity with Android airplane: load once, CPU, n_ctx 512, short reply.
    #if !os(macOS)
    #if canImport(llama)
    private static var iosHeartCtx: LlamaContext?
    private static var iosHeartPath: String?
    private static let iosHeartGate = NSLock()
    #endif
    ...
    private static func generateIOS(user: String) -> String {
        ...
        let path = heart.path
        // Short system like Android NativeHeart — Standing law stays on Mac CLI path.
        let sys = "You are ЯBOT. One short offline reply. Offline NonNuclear."
        let prompt = sys + "\nUser: " + user + "\nAssistant:"
        ...
        Task.detached(priority: .userInitiated) {
            do {
                // Persistent load (Android loads once into filesDir engine).
                let ctx: LlamaContext
                iosHeartGate.lock()
                if let existing = iosHeartCtx, iosHeartPath == path {
                    ctx = existing
                    iosHeartGate.unlock()
                    await ctx.clear()
                } else {
                    iosHeartGate.unlock()
                    let fresh = try await LlamaContext.create_context(path: path)
                    iosHeartGate.lock()
                    iosHeartCtx = fresh
                    iosHeartPath = path
                    iosHeartGate.unlock()
                ...
                try await ctx.completion_init(text: prompt)
                var out = ""
                for _ in 0..<maxNew {
        ...
        // First cold load of ~2.1GB can be slow on CPU; warm generates stay short.
        if sem.wait(timeout: .now() + 240) == .timedOut {
            return "MISSING — Heart timed out (~240s). First load is heavy; retry once."
        }
