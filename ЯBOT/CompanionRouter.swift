...
enum CompanionRouter {
    ...
    Dual tongue (forever):
    • Decider: short orders (ping, think, commands, revert, reform, respawn, evolve, mint, yacode, workshop…)
    ...
    static let gameWorkshopInfoWords: Set<String> = [
        "workshop", "game workshop", "game builders workshop", "builders workshop", "gamewrite", "game write",
        "gamewrite help", "gamewrite languages", "gamewrite list", "game drafts", "workshop drafts", "workshop status",
    ]

    static func isGameWorkshopInfo(_ lower: String) -> Bool {
        gameWorkshopInfoWords.contains(lower) || lower.hasPrefix("gamewrite show ")
            || lower.hasPrefix("game approve") || lower.hasPrefix("game apply") || lower.hasPrefix("game reject")
            || lower.hasPrefix("game respawn") || lower.hasPrefix("workshop respawn")
    }

    static func isGameWriteDraft(_ lower: String) -> Bool {
        for p in ["gamewrite ", "gamewrite:", "game write ", "game write:", "write game code", "draft game code", "write game ", "draft game "] {
    ...
    static func openWorkshopUI() {
        DispatchQueue.main.async {
            NotificationCenter.default.post(name: Notification.Name("ЯBOT.OpenGameWorkshop"), object: nil)
        }
    }
...
static func classify(_ raw: String) -> ChatLane {
        ...
        // GAME BUILDERS WORKSHOP (0.3.0): info = FUNCTION · drafting code = ACTION (writes a draft)
        if isGameWorkshopInfo(lower) { return .function }
        if isGameWriteDraft(lower) { return .action }
    ...
    private static func replyText(to raw: String, isOnline: Bool) -> String {
        ...
        if lower == "label teach" || lower == "bot teach" || lower == "labels teach" {
            return """
            BOT LABEL HOW (naming law 2026-09-24 — Garage names):
            • U = the human · Я = the Machine Mind, all components behind it (never numbered)
            • bots = Я's extensions / digital robot kids — each speaks with its designated Garage name
            • Garage roster now: \(BotLabel.rosterNamesLine())
            • bot enter <Name> — a Garage bot speaks (stamps = its Garage name); unknown names refused
            • bot mint / bot create / bot new <Name> — new bot in the Garage roster (name required; no numbers shown)
            • bot leave — assistant stamps = Я again · bot enter Я also returns to the Machine Mind
            • legacy titles (before the law, e.g. ЯBOT#2 · ЯBOT#3) are kept as-is — only their own origin (device/IP) may use them
            • who / labels — the law + the Garage roster + legacy titles with origin
            Example: bot enter ЯMAX → talk → bot leave
            """
        }
        ...
        if lower == "bot enter" {
            return BotLabel.enter("")  // HOW + Garage roster — no anonymous join, no auto numbers
        }
        ...
        if lower == "commands" || lower == "help" || lower == "functions" {
            ...
            12  respawn — list known-good respawn points + exact undo command (template: respawn template) — 7/10
            ...
            43  who — U · Я (Machine Mind) · Garage bots by name — 8/10
            44  bot — create / enter / leave Garage-named bots (Я's kids) — 8/10
            ...
            · gamewrite <language> <file> <purpose> · gamewrite list|languages|show <id> · respawn template
        ...
        if lower == "bolte" || lower == "being" || lower == "rzl being" {
            // MERGE 0.3.0 (Documents hunk): Bolte face opens the one fixed manual ЯMANUAL (no new base command).
            let opened = ManualPDFLocator.openSeatedPDF()
            let openLine = opened ? "Opened ЯMANUAL (one fixed app manual)." : "ЯMANUAL.pdf not seated — seat mind/books/YAMANUAL-0.1.pdf"
            return "BOLTE · ЯMANUAL face.\n\(openLine)\nBOLTE is the first RZL Being face — freeform claymation underworld cloud + lightning. Can evolve forms/emotions when Decider grants. Still NonNuclear."
        }
        ...
        if lower == "revert" || lower == "reform" {
            return "\(lower) noted. For a one-step undo of a build say RESPAWN (lists known-good respawn points + exact command). Game projects: Workshop → Respawn. Snapshot first before a risky mod."
        }

        // RESPAWN (0.3.0): list known-good respawn points + exact undo command. Never self-applies.
        if lower == "respawn" || lower == "command: respawn" || lower == "command respawn" || lower == "respawn points" || lower == "respawn list" {
            return RespawnPoints.status()
        }
        // Older meaning kept: restore most recent official template into Documents/ЯBOT/ЯBOT — Decider confirm required.
        if lower == "respawn template" {
            return "RESPAWN TEMPLATE overwrites Documents/ЯBOT/ЯBOT from the most recent official template (ya-respawn.sh · rsync --delete).\nNonNuclear: say `respawn template confirm` to run it. Prefer `respawn` (build respawn points) for undoing a build."
        }
        if lower == "respawn template confirm" {
            return TemplateRespawn.runMostRecentOfficial()
        }
        ...
        // HARDCODE 2026-09-23: base `manual` / `yamanual` opens ЯMANUAL only (not a menu of leaves).
        // Lab desk stays extension: `lab manual`.
        if lower == "manual" || lower == "yamanual" || lower == "яmanual" || lower == "open manual" || lower == "giver manual" || lower == "user giver" {
            let ok = ManualPDFLocator.openFixedManual()
            if ok {
                return "ЯMANUAL · one fixed app manual · opened / seated in MACHINE MIND"
            }
            return "ЯMANUAL.pdf not seated — seat mind/books/YAMANUAL-0.1.pdf"
        }
        ...
        // GAME BUILDERS WORKSHOP + GAMEWRITE (0.3.0) — authors ЯBOT + ЯMAX (authors.json) draft; Decider applies in the Workshop.
        if lower == "workshop" || lower == "game workshop" || lower == "game builders workshop" || lower == "builders workshop" || lower == "workshop status" {
            openWorkshopUI()
            return GameWorkshop.status() + "\nOpening GAME BUILDERS WORKSHOP…"
        }
        if lower == "gamewrite" || lower == "game write" || lower == "gamewrite help" {
            return GameWrite.help
        }
        if lower == "gamewrite languages" {
            return GameWrite.languagesLine()
        }
        if lower == "gamewrite list" || lower == "game drafts" || lower == "workshop drafts" {
            return GameWrite.listDrafts()
        }
        if lower.hasPrefix("gamewrite show ") {
            return GameWrite.show(String(text.dropFirst("gamewrite show ".count)).trimmingCharacters(in: .whitespaces))
        }
        if lower.hasPrefix("game approve") || lower.hasPrefix("game apply") || lower.hasPrefix("game reject")
            || lower.hasPrefix("game respawn") || lower.hasPrefix("workshop respawn") {
            openWorkshopUI()
            return "Decider only: Approve & Apply, Reject and Respawn are taps inside the GAME BUILDERS WORKSHOP (not chat words), so no bot, inbox line or link can apply game code. Opening the Workshop…"
        }
        if isGameWriteDraft(lower) {
            return GameWrite.handle(text)
        }
        if lower == "game" || lower == "teraform" || lower == "teraformя" || lower == "я game" || lower == "yagame" {
            return "Я Game door: yabot://game · GAME BUILDERS WORKSHOP: yabot://game/workshop (say: workshop)\nPrefers local Documents/ЯBOT/game/index.html · else https://rizal.info/game/ (http fallback)\nNonNuclear: door only — no spend / auto-rewards / mint.\nCrown Я · mint ref \(YaTwinChain.mint)"
        }
