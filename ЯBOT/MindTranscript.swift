...
/// U (biological) · Я (clay seat) · guest bots · CoS · Rizalbot · successors · tools/code notes.
...
enum MindTranscript {
    ...
    static func append(role: String, kind: String, body: String, surface: String? = nil, party: String? = nil) -> Bool {
        ...
        let who = party ?? BotLabel.clayLabel
    ...
    private static func writeAppend(_ data: Data, to url: URL) -> Bool {
        ...
        if FileManager.default.fileExists(atPath: url.path) {
            ...
            # Parties: U | Я | guest-bots | CoS | Rizalbot | successor | system  (U=biological · Я=clay seat)
...
/// CoS/device-smoke inject: drop lines into Application Support/ЯBOT/mind/INBOX-COMMANDS.txt
/// (one command per line). Documents mirror only after Files & Folders grant.
enum ClayCommandInbox {
    ...
    /// Primary + iCloud / App Support mirrors (SHOT macos↔ios relay).
    static var drainURLs: [URL] {
        LabShotCommand.relayInboxURLs()
    }
    ...
    /// One clay inbox line → one handler, even when many WindowGroups subscribe.
    private static let consumeLock = NSLock()
    private static var lastConsumed: (text: String, at: Date)?
    static func consumeIfFirst(_ text: String, _ body: () -> Void) {
        consumeLock.lock()
        defer { consumeLock.unlock() }
        let now = Date()
        if let last = lastConsumed,
           last.text == text,
           now.timeIntervalSince(last.at) < 1.5 {
            return
        }
        lastConsumed = (text, now)
        body()
    }
