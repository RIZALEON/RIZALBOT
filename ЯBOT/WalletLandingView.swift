...
/// BOTTOM-CHROME-STRIP: no wallet/house/Mind bottom nav (ЯBAR Home/Wallet/Mind own leave).
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 16) {
                ...
                // MAGNET UNDER BAR — tablet header never overlaps sticky ЯBAR
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        row("Mint", card.mint)
                        // Page actions (NOT bottom nav) — copy mint / open explorer stay as tablet links
                        HStack(spacing: 14) {
                            Button("Copy mint") { copyMint() }
                                .font(ClayTheme.clayFont(size: 12, weight: .bold))
                                .foregroundStyle(Color.orange.opacity(0.95))
                                .buttonStyle(.plain)
                                .help("Copy mint address")
                            if isOnline, let url = URL(string: card.explorer ?? "") {
                                Button("Open explorer") { openURL(url) }
                                    .font(ClayTheme.clayFont(size: 12, weight: .bold))
                                    .foregroundStyle(Color.orange.opacity(0.95))
                                    .buttonStyle(.plain)
                        ...
                        row("Channel", "\(YaTwinChain.channelName) · \(YaTwinChain.channelHandle)")
                        if isOnline, let channel = URL(string: YaTwinChain.channelURL) {
                            Link("Open CHANNEL R on YouTube", destination: channel)
                                .font(ClayTheme.clayFont(size: 12, weight: .medium))
                        } else {
                            row("YouTube", YaTwinChain.channelURL)
                        }
                        row("Channel association", "Decider-provided · local reference")
