...
#else
/// iOS twin stub — SelfFix Lab is Mac/Process only (NonNuclear; Decider rebuilds).
enum SelfFix {
    static func run(_ kind: String = "") -> String { "self-fix unavailable on iOS twin" }
    static func diagnose() -> String { "n/a" }
    static func list() -> String { "SELF-FIX lab is Mac-only (Process + Documents CODE-LAB)." }
    static func authorAndPropose(brief: String) -> String {
        "SELF-FIX author is Mac-only. Brief parked: \(brief.prefix(120))"
    }
    static func apply(name: String = "") -> String {
        "SELF-FIX apply is Mac-only. Name: \(name)"
    }
    static func propose(brief: String, source: String, name: String? = nil) -> String {
        "SELF-FIX propose is Mac-only."
    }
    static func proposeShell(brief: String, script: String, name: String) -> String {
        "SELF-FIX shell propose is Mac-only."
    }
    static func runShell(name: String) -> String {
        "SELF-FIX shell run is Mac-only. Name: \(name)"
    }
}
#endif
