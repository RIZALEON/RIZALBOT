...
/// Claymation landings.
/// Bolte / USER MANUAL: Manual cover → download PDF; Home on sticky ЯBAR leaves landing.
...
struct ClayLandingView: View {
    ...
    private func downloadManual() {
        ...
        #if canImport(AppKit)
        // MERGE 0.3.0 (Documents hunk): prefer open/view for the one fixed manual.
        if NSWorkspace.shared.open(src) {
            flash("Opened ЯMANUAL")
            return
        }
