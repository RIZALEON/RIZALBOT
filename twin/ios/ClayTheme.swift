...
enum ClayImage {
    /// Hot-assets: App Support first (TCC sticky); Documents only when ya.tcc.documents.granted.
    static var hotAssetsDirectory: URL {
#if os(macOS)
        FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent("Library/Application Support/ЯBOT/hot-assets", isDirectory: true)
#else
        FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("ЯBOT/hot-assets", isDirectory: true)
#endif
    }

    static var documentsHotAssetsDirectory: URL {
#if os(macOS)
        FileManager.default.homeDirectoryForCurrentUser
    ...
    static func hotAssetURL(named name: String) -> URL? {
        ...
        var dirs = [hotAssetsDirectory]
        if UserDefaults.standard.bool(forKey: "ya.tcc.documents.granted") {
            dirs.append(documentsHotAssetsDirectory)
        }
        for dir in dirs {
            for c in candidates {
