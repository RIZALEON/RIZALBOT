...
struct ContentView: View {
    ...
    private var chromeOverlays: some View {
        VStack(spacing: 0) {
            HStack(alignment: .center, spacing: 10) {
                ...
                // Wallet + mode + mind — same size, center-aligned (far upper right).
                HStack(alignment: .center, spacing: 8) {
                    ClayButton(
                        asset: "BtnWallet",
                        ...
                        width: 32,
                        height: 32,
                        help: "Я wallet — landing (on/offline)"
                    ) {
                        withAnimation(.easeInOut(duration: 0.18)) {
    ...
    private func send() {
        ...
        let turn = CompanionRouter.handle(to: routeInput, isOnline: isOnline)
        let lane = turn.lane
        remember(ChatMessage(role: .user, text: userLine, lane: lane))
        MindTranscript.append(role: "user", kind: "prompt-\(lane.rawValue)", body: userLine, party: "Decider")
        draft = ""
        attachments = []
        ...
        MindTranscript.append(role: "assistant", kind: "reply-\(lane.rawValue)", body: fileAck, party: "clay")
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.12) {
            remember(ChatMessage(role: .assistant, text: fileAck, lane: lane))
        }
    }
